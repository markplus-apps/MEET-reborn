...
export async function sendBookingEmail(data: BookingEmailData) {
  ...
  const statusConfig = {
    CONFIRMED: { label: "Booking Confirmed", message: "Your meeting room booking has been confirmed.", gradient: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", badgeBg: "#e8f5e9", badgeColor: "#2e7d32" },
    MODIFIED: { label: "Booking Modified", message: "Your meeting room booking has been updated with new details.", gradient: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", badgeBg: "#e3f2fd", badgeColor: "#1565c0" },
    CANCELLED: { label: "Booking Cancelled", message: "Your meeting room booking has been cancelled.", gradient: "linear-gradient(135deg, #f093fb 0%, #f5576c 100%)", badgeBg: "#fce4ec", badgeColor: "#c62828" },
  };
  const cfg = statusConfig[data.status];
  const subject = `${cfg.label}: ${data.title} - ${data.roomName}`;
  ...
  const html = `
    ...
    <head>
      <style>
        ...
        .header { background: ${cfg.gradient}; padding: 40px 32px; text-align: center; }
        ...
        .status { display: inline-block; padding: 6px 16px; border-radius: 20px; font-size: 13px; font-weight: 600; background: ${cfg.badgeBg}; color: ${cfg.badgeColor}; }
